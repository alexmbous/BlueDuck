...
public class BaseKeyLockServiceActivity extends Activity {
     ...
     * 操作 设置开锁键盘密码
     */
    protected  void sendKeyLockInputPWD(byte index,long password  ){
        mService.sendInputPassword(index,password);
    }
    /**
     * 操作 设置密码键盘的参数
    ...
    protected  void onBLEKeyLockInputPassword(int total,int status){
...
//                Log.i(TAG, "onReceive:info  thread name="+Thread.currentThread().getName());
                ...
                onBLEKeyLockInputPassword(  total,  status);
            }else if(KeyLockService.ACTION_BLE_KEY_LOCK_OPEN_RECORD.equals(intent.getAction())){
                long time = intent.getLongExtra(KeyLockService.EXTRA_RECORD_TIME,0L);
                ...
                onBLEKeyLockOpenRecordCallback(  recordIndex,  timestamp,time);
            }
        }
    };
}
