...
//                    handler.sendEmptyMessage(HANDLER_BLE_ON_CONNECT);
    ...
    private void handOpenRecord(byte[] command){
        byte recordIndex = command[6];
        long openTimestamp=((command[7]&0xFF)<<24 )|((command[8]&0xFF)<<16)|((command[9]&0xFF)<<8)| (command[10]&0xFF);
        long openTime =((command[11]&0xFF)<<24 )|((command[12]&0xFF)<<16)|((command[13]&0xFF)<<8)| (command[14]&0xFF);
    ...
    public byte[] sendInputPassword(byte index,long password ){
        Log.i(TAG, "sendInputPassword: 写入备份密码");

        byte[] crcOrder= KeyLockCommand.getCRCInputPWD(mBLECommunicationKey,index,password);
